...
```
Agent 修改了 docs/guide/mcp/index.md
  → 当前文件的链接检查通过 ✓
  → 但是其他页面引用了旧路径 /guide/mcp/intro → 404 ✗
  → 没有回归套件 → 这个问题到用户访问时才发现
```
...
```
Agent 修改了 docs/guide/mcp/index.md
  → L3 回归检查: npm run docs:check-links → 3 个 404
  → Agent 收到反馈: "这些页面引用了旧路径"
  → Agent 修复受影响的页面
  → 重新检查 → 0 个 404 → 可以提交 PR
```
...
快速门禁:
  成本: ~5s（链接解析 + HEAD 请求）
  收益: 拦截 80% 的常见错误（断链、格式错误）
  触发频率: 每次 write_file 后

标准门禁:
  成本: ~3min（完整构建 + 全量检查）
  收益: 拦截 15% 的中等问题（构建失败、路由错误）
  触发频率: 每次 PR 提交前

全量门禁:
  成本: ~10min（Lighthouse + 人工截图）
  收益: 拦截 5% 的低频但高影响问题（a11y 退化、移动端溢出）
  触发频率: 每次发布前
```
...
最常见的错误是 **Golden Dataset 过时了**：

```
问题: PR 每次都报失败，但实际没问题
原因: 3 个月前新增了一个模块，但 expected-routes.json 没更新
修复: 运行快照更新命令，提交新的 golden 数据
```

Golden Dataset 不是"一次写好就不管了"——它需要随项目演进同步更新。
...
漂移不一定意味着回归——可能是正常的项目增长。区分两者的关键是**变化率**和**变化方向**：

```
正常增长: 页面数量从 100 → 103（新增 3 页，符合 PR 描述）
回归:     页面数量从 100 → 97（有 3 页丢失了！）

正常增长: 构建时间从 6.2s → 6.8s（新增了页面，合理）
回归:     构建时间从 6.2s → 45s（可能是死循环或无限递归）
```

### 趋势监控

除了单次对比，回归套件还应该记录历史趋势：

```
日期        页面数  构建时间  断链数  Bundle大小
2024-01-01  100     6.2s     0       45KB
2024-01-15  103     6.4s     0       46KB   ← 正常增长
2024-02-01  105     6.8s     0       47KB   ← 正常增长
2024-02-15  104     12.3s    2       48KB   ← 构建时间异常，需要调查
```

趋势数据比单次快照更有价值——它能发现"温水煮青蛙"式的缓慢退化。
...
- **失败必须阻止合并**：如果回归检查失败，PR 不能合并。这不是建议，是强制。
- **反馈必须快速**：CI 运行时间 > 10 分钟会导致开发者忽略结果。保持标准门禁 < 5 分钟。
- **结果必须可解读**：CI 输出必须包含具体失败原因，不能只是一个红色叉号。
...
npm run docs:check-stale        # 检查是否有未归档的僵尸变更
...
```
Step 1: 识别关键检查点
  - 项目最常见的失败模式是什么？（断链、构建失败、格式错误）
  - 这些失败的检测成本是什么？（时间、复杂度）

Step 2: 从快速门禁开始
  - 写 1-2 个最简单的 evaluator（如链接检查）
  - 确保能在 30 秒内完成

Step 3: 添加标准门禁
  - 加入完整构建检查
  - 加入 Golden Dataset 快照对比
...
Step 5: 持续维护
  - 每次项目结构变化时更新 Golden Dataset
  - 定期审查 evaluator 是否仍然有效
```
...
| 回归套件运行太慢（> 10min） | 检查项太多或某些检查效率低 | 分层：把慢检查移到全量门禁，优化瓶颈检查 |
| 某个 evaluator 间歇性失败 | 检查依赖外部资源（如网络请求） | 添加重试机制或 mock 外部依赖 |
